...
function App() {
  ...
  const renderContent = () => {
    ...
    // The main Growth tab is locked as it displays private historical trends
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-slate-900 flex flex-col md:flex-row font-sans selection:bg-primary/30">
      ...
      {/* System-style Passcode Modal */}
      {showPasscodeModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/40 backdrop-blur-3xl animate-fade-in">
          <div className="relative w-full h-full md:h-auto md:max-w-md bg-white/90 dark:bg-slate-900/95 md:rounded-[48px] p-10 flex flex-col items-center justify-center shadow-2xl border border-white/20">
            
            {/* Close/Back button for mobile */}
            <button 
                onClick={() => setShowPasscodeModal(false)} 
                className="absolute top-8 left-8 p-3 text-slate-400 hover:text-slate-600 transition-colors md:hidden"
            >
                <ChevronLeft className="w-8 h-8"/>
            </button>

            <div className="mb-12 flex flex-col items-center">
                <div className="w-20 h-20 bg-primary/10 rounded-[28px] flex items-center justify-center text-primary mb-6">
            ...
            {/* System Keypad Layout */}
            <div className="grid grid-cols-3 gap-8 mb-10">
              {[1,2,3,4,5,6,7,8,9].map(num => (
                  <button 
                    key={num} 
                    onClick={() => passcodeInput.length < 4 && setPasscodeInput(p => p + num)} 
                    className="w-20 h-20 rounded-full bg-slate-50 dark:bg-slate-800/50 text-3xl font-bold text-slate-700 dark:text-slate-100 hover:bg-primary hover:text-white active:scale-90 transition-all shadow-sm"
                  >
                      {num}
                  </button>
              ))}
              <div className="w-20 h-20"></div> {/* Empty spacer */}
              <button 
                onClick={() => passcodeInput.length < 4 && setPasscodeInput(p => p + '0')} 
                className="w-20 h-20 rounded-full bg-slate-50 dark:bg-slate-800/50 text-3xl font-bold text-slate-700 dark:text-slate-100 hover:bg-primary hover:text-white active:scale-90 transition-all shadow-sm"
              >
                  0
              </button>
              <button 
                ...
                className="w-20 h-20 flex items-center justify-center text-slate-300 hover:text-rose-500 active:scale-90 transition-all"
              >
                  <Delete className="w-10 h-10"/>
              </button>
            </div>

            <button onClick={() => setShowPasscodeModal(false)} className="text-sm font-bold text-slate-400 hover:text-slate-600 py-2">
                {t('cancel_btn')}
            </button>
          </div>
        </div>
