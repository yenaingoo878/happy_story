...
import { Home, PlusCircle, BookOpen, Activity, Camera, Image as ImageIcon, Baby, ChevronRight, Sparkles, Plus, Moon, Sun, Pencil, X, Settings, Trash2, ArrowLeft, Ruler, Scale, Calendar, Lock, Unlock, ShieldCheck, KeyRound, Cloud, CloudOff, RefreshCw, AlertTriangle, Save, UserPlus, LogOut, User } from 'lucide-react';
...
function App() {
  ...
  const handleEditStart = (memory: Memory) => { setNewMemory({ title: memory.title, desc: memory.description, imageUrl: memory.imageUrl, date: memory.date }); setEditingId(memory.id); setActiveTab(TabView.ADD_MEMORY); setSettingsView('MAIN'); setSelectedMemory(null); };
  const handleCancelEdit = () => { setNewMemory({ title: '', desc: '', date: getTodayLocal() }); setEditingId(null); setActiveTab(TabView.HOME); };
  ...
  const handleSaveMemory = async () => { if (!newMemory.title || !activeProfileId) return; const finalImageUrl = newMemory.imageUrl || `https://picsum.photos/400/300?random=${Date.now()}`; if (editingId) { const existing = memories.find(m => m.id === editingId); if (existing) { const updated: Memory = { ...existing, childId: existing.childId, title: newMemory.title, description: newMemory.desc, imageUrl: finalImageUrl, date: newMemory.date }; await DataService.addMemory(updated); } } else { const memory: Memory = { id: generateId(), childId: activeProfileId, title: newMemory.title, description: newMemory.desc, date: newMemory.date, imageUrl: finalImageUrl, tags: ['New Memory'], synced: 0 }; await DataService.addMemory(memory); } await loadChildData(activeProfileId); setNewMemory({ title: '', desc: '', date: getTodayLocal() }); setEditingId(null); setActiveTab(TabView.HOME); };
  const handleAddGrowthRecord = async () => { if (newGrowth.month !== undefined && newGrowth.height && newGrowth.weight && activeProfileId) { let updatedData: GrowthData = { id: newGrowth.id || generateId(), childId: activeProfileId, month: Number(newGrowth.month), height: Number(newGrowth.height), weight: Number(newGrowth.weight), synced: 0 }; await DataService.saveGrowth(updatedData); await loadChildData(activeProfileId); setNewGrowth({ month: undefined, height: undefined, weight: undefined }); setIsEditingGrowth(false); } };
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case TabView.ADD_MEMORY:
        // ... (Keep existing ADD_MEMORY logic, essentially just returning JSX)
        return (
          <div className="pb-32 animate-fade-in">
             {/* Copy existing ADD_MEMORY content exactly */}
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 transition-colors">
              <div onClick={triggerFileInput} className="relative w-full h-48 md:h-64 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-600 mb-6 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors group overflow-hidden">
                {newMemory.imageUrl ? (<><img src={newMemory.imageUrl} alt="Preview" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><div className="bg-white/20 backdrop-blur-md p-2 rounded-full text-white"><Camera className="w-6 h-6" /></div></div></>) : (<div className="flex flex-col items-center justify-center w-full h-full"><div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-600 flex items-center justify-center text-slate-400 dark:text-slate-300 group-hover:bg-white dark:group-hover:bg-slate-500 group-hover:text-primary transition-colors"><Camera className="w-6 h-6" /></div><p className="mt-2 text-sm text-slate-400 dark:text-slate-400">{t('choose_photo')}</p></div>)}
                {/* Added capture="environment" for better mobile camera support */}
                <input ref={fileInputRef} type="file" accept="image/*" capture="environment" onChange={handleImageUpload} className="hidden" />
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">{t('form_title')}</label><input type="text" value={newMemory.title} onChange={(e) => setNewMemory({...newMemory, title: e.target.value})} placeholder={t('form_title_placeholder')} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors"/></div><div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">{t('date_label')}</label><input type={dateInputType} value={dateInputType === 'date' ? newMemory.date : formatDateDisplay(newMemory.date)} onFocus={() => setDateInputType('date')} onBlur={() => setDateInputType('text')} onChange={(e) => setNewMemory({...newMemory, date: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors"/></div></div>
                <div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">{t('form_desc')}</label><textarea value={newMemory.desc} onChange={(e) => setNewMemory({...newMemory, desc: e.target.value})} placeholder={t('form_desc_placeholder')} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none h-32 resize-none transition-colors"/></div>
                <div className="flex gap-3 pt-2">{editingId && (<button onClick={handleCancelEdit} className="flex-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold py-3.5 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors active:scale-95">{t('cancel_btn')}</button>)}<button onClick={handleSaveMemory} className={`${editingId ? 'flex-[2]' : 'w-full'} bg-primary hover:bg-rose-400 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-primary/30 transition-all active:scale-95`}>{editingId ? t('update_btn') : t('record_btn')}</button></div>
              </div>
            </div>
          </div>
        );
                ...
                <div className="grid grid-cols-1 gap-4 mt-2">
                  {!isDetailsUnlocked ? (
                    ...
                    <div className="space-y-4 animate-fade-in mt-2 relative">
                        ...
                        <div className="relative"><label className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 absolute top-2 left-3">{t('child_name')}</label><input type="text" value={editingProfile.name} onChange={(e) => setEditingProfile({...editingProfile, name: e.target.value})} className="w-full px-3 pb-2 pt-6 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors font-medium text-sm" placeholder="Baby Name" /></div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="relative"><label className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 absolute top-2 left-3">{t('child_dob')}</label><input type={dobInputType} value={dobInputType === 'date' ? editingProfile.dob : formatDateDisplay(editingProfile.dob)} onFocus={() => setDobInputType('date')} onBlur={() => setDobInputType('text')} onChange={(e) => setEditingProfile({...editingProfile, dob: e.target.value})} className="w-full px-3 pb-2 pt-6 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors font-medium text-sm" /></div><div className="relative"><label className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 absolute top-2 left-3">{t('child_birth_time')}</label><input type="time" value={editingProfile.birthTime || ''} onChange={(e) => setEditingProfile({...editingProfile, birthTime: e.target.value})} className="w-full px-3 pb-2 pt-6 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors font-medium text-sm" /></div><div className="relative"><label className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 absolute top-2 left-3">{t('hospital_name')}</label><input type="text" value={editingProfile.hospitalName || ''} onChange={(e) => setEditingProfile({...editingProfile, hospitalName: e.target.value})} className="w-full px-3 pb-2 pt-6 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors font-medium text-sm" placeholder={t('hospital_placeholder')} /></div><div className="relative"><label className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 absolute top-2 left-3">{t('birth_location')}</label><input type="text" value={editingProfile.birthLocation || ''} onChange={(e) => setEditingProfile({...editingProfile, birthLocation: e.target.value})} className="w-full px-3 pb-2 pt-6 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors font-medium text-sm" placeholder={t('location_placeholder')} /></div></div>
                         <div className="flex gap-3 mt-2">{editingProfile.id && (<button onClick={() => requestDeleteProfile(editingProfile.id || '')} className="flex-1 py-3 rounded-xl bg-rose-50 dark:bg-rose-900/20 text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/40 font-bold text-sm transition-all">{t('delete')}</button>)}<button onClick={handleSaveProfile} className="flex-[2] py-3 rounded-xl bg-primary hover:bg-rose-400 text-white font-bold text-sm shadow-md transition-all active:scale-95 flex items-center justify-center"><Save className="w-4 h-4 mr-2" />{t('save_changes')}</button></div>
                    </div>
                  )}
                </div>
             </div>
