...
function App() {
        ...
        {particles.map(p => (
          <div 
            ...
            style={{ 
              ...
              '--tw-translate-x': p.type === 'boom' ? (p as any).tx : '0',
              '--tw-translate-y': p.type === 'boom' ? (p as any).ty : '0',
  ...
  const handleLogout = async () => {
      ...
      try { if (session) await supabase.auth.signOut(); } finally {
      ...
      case TabView.HOME:
        ...
        return (
          <div className="space-y-4 pb-32 md:pb-8 animate-fade-in max-w-7xl mx-auto">
            ...
            {/* Celebration & Reminders Header */}
            {remindersEnabled && (
                <div className="space-y-2 mb-4">
                    {/* Birthday Alert */}
                    {showBirthdayBanner && bStatus !== 'NONE' && (
                       <div className={`rounded-2xl p-4 text-white shadow-md relative animate-zoom-in ${bStatus === 'TODAY' ? 'bg-gradient-to-r from-rose-400 to-pink-500' : 'bg-gradient-to-r from-amber-400 to-orange-400'}`}>
                    ...
                    {activeReminders.map(rem => (
                        <div key={rem.id} className="bg-white dark:bg-slate-800 border-l-4 border-primary rounded-xl p-4 shadow-sm flex items-center justify-between animate-slide-up">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-primary/10 rounded-lg text-primary"><Bell className="w-4 h-4"/></div>
                                <div><h4 className="font-bold text-sm text-slate-700 dark:text-slate-200">{rem.title}</h4><p className="text-xs text-slate-400">{t('nav_story')}</p></div>
                            </div>
                        </div>
                    ))}
                </div>
