...
export const GalleryGrid: React.FC<GalleryGridProps> = ({ memories, language, onMemoryClick, userId, activeProfileId, requestDeleteConfirmation }) => {
  ...
  return (
    <div className="pb-24 animate-fade-in relative">
       ...
       {previewState.url && (<div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl animate-fade-in"><div className="absolute inset-0" onClick={() => setPreviewState({ url: null, data: null, isLoading: false })} /><button onClick={() => setPreviewState({ url: null, data: null, isLoading: false })} className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors z-[210]"><X className="w-6 h-6" /></button>{previewState.isLoading ? (<Loader2 className="w-12 h-12 text-white animate-spin" />) : (<img src={previewState.data || previewState.url} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl animate-zoom-in relative z-[205]" alt="Preview" />)}<button onClick={(e) => { e.stopPropagation(); requestDeleteConfirmation(async () => { if(previewState.url) { await DataService.deleteCloudPhoto(previewState.url); setPreviewState({ url: null, data: null, isLoading: false }); await fetchCloudPhotos(); } }); }} className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[210] px-6 py-4 bg-rose-500/80 hover:bg-rose-600 backdrop-blur-md text-white rounded-2xl flex items-center gap-3 font-black text-sm uppercase tracking-widest active:scale-95 transition-all shadow-lg"><Trash2 className="w-5 h-5" />{t('delete')}</button></div>)}
    </div>
  );
};