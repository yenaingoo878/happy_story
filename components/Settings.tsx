    ...
    <div className="max-w-4xl mx-auto px-2 relative">
      ...
      {view === 'MAIN' && (
        <div className="animate-fade-in space-y-6 pb-32">
          ...
          {/* User Profile Card - Updated for better spacing and layout */}
          <section className="bg-white dark:bg-slate-800 rounded-[32px] overflow-hidden shadow-xl border border-slate-100 dark:border-slate-700 p-6">
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 px-1 no-scrollbar border-b border-slate-50 dark:border-slate-700/50 mb-6 items-center">
                {profiles.map(p => (
                  <button 
                    key={p.id} 
                    onClick={() => onProfileChange(p.id!)} 
                    className={`flex-shrink-0 flex flex-col items-center gap-2.5 transition-all duration-300 ${p.id === activeProfileId ? 'scale-105' : 'opacity-40 grayscale'}`}
                  >
                    <div className={`w-10 h-10 rounded-[15px] border-2 overflow-hidden flex items-center justify-center ${p.id === activeProfileId ? 'border-primary ring-4 ring-primary/10 shadow-lg' : 'border-transparent bg-slate-100 dark:bg-slate-700'}`}>
                      {p.profileImage ? <img src={p.profileImage} className="w-full h-full object-cover" /> : <Baby className="w-4 h-4 text-slate-400" />}
                    </div>
            ...
            <div className="flex items-center justify-between px-1">
                <div className="text-left">
                    <h2 className="text-xl font-black text-slate-800 dark:text-white tracking-tight leading-none mb-1.5">{currentProfile?.name}</h2>
                    <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">{currentProfile?.dob}</p>
                </div>
                <button onClick={() => isLocked ? onUnlockRequest() : setShowProfileDetails(!showProfileDetails)} className="flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all bg-primary text-white active:scale-95 shadow-lg shadow-primary/20">{isLocked ? <Lock className="w-3.5 h-3.5" /> : (showProfileDetails ? <X className="w-3.5 h-3.5" /> : <Pencil className="w-3.5 h-3.5" />)}{isLocked ? t('tap_to_unlock') : (showProfileDetails ? t('close_edit') : t('edit_profile'))}</button>
            </div>

            {showProfileDetails && !isLocked && (
              <div className="animate-slide-up space-y-4 pt-6 pb-2 overflow-y-auto max-h-[70vh] no-scrollbar">
          ...
          {/* Cloud Sync and other sections omitted for brevity but remain identical to the previous version */}
        </div>
      )}
    </div>
  );
