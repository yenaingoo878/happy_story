...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const handleSaveProfile = async () => {
    ...
    try {
        ...
        await onRefreshData();
        setShowProfileDetails(false); triggerSuccess();
    ...
    <div className="max-w-4xl mx-auto px-2 relative">
      ...
      {view === 'MAIN' && (
        <div className="animate-fade-in space-y-5 pb-32">
          <div className="flex items-center justify-between px-2"><div className="text-left"><h1 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">{t('settings_title')}</h1><p className="text-slate-500 dark:text-slate-400 font-bold text-sm mt-0.5">{t('settings_subtitle')}</p></div><div className="w-10 h-10 bg-primary/10 rounded-2xl flex items-center justify-center text-primary"><SettingsIcon className="w-5 h-5"/></div></div>
          <section className="bg-white dark:bg-slate-800 rounded-3xl p-4 border border-slate-100 dark:border-slate-700 flex items-center justify-between transition-all">
             <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center text-emerald-500"><Cloud className="w-4 h-4" /></div><div className="text-left"><h3 className="text-[11px] font-black text-slate-800 dark:text-white">Cloud Backup</h3><p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">{isGuestMode ? 'Saved on Phone' : 'Synced'}</p></div></div>
             <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-700/50 rounded-xl flex items-center gap-2 border border-slate-100 dark:border-slate-600/30 shadow-inner"><HardDrive className="w-3.5 h-3.5 text-slate-400" /><span className="text-[9px] font-black text-slate-500 dark:text-slate-300 uppercase tracking-widest">{memories.length + stories.length + growthData.length} Records</span></div>
          </section>
          <section className="bg-white dark:bg-slate-800 rounded-[32px] overflow-hidden shadow-xl border border-slate-100 dark:border-slate-700 p-5">
            <div className="flex items-center justify-between mb-4 px-1"><div className="flex items-center gap-2.5"><CircleUser className="w-4 h-4 text-slate-400"/><h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{t('about_child')}</h3></div><button onClick={() => DataService.saveProfile({ id: crypto.randomUUID(), name: t('add_new_profile'), dob: new Date().toISOString().split('T')[0], gender: 'boy' }).then(() => onRefreshData())} className="flex items-center gap-1.5 text-primary text-[10px] font-black uppercase tracking-wider px-3 py-1.5 bg-primary/5 rounded-xl"><Plus className="w-3.5 h-3.5"/> {t('add_new_profile')}</button></div>
            ...
            <div className="space-y-4"><div className="flex items-center justify-between px-1"><div className="text-left"><h2 className="text-xl font-black text-slate-800 dark:text-white tracking-tight">{currentProfile?.name}</h2><p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mt-1.5">{currentProfile?.dob}</p></div><button onClick={() => isLocked ? onUnlockRequest() : setShowProfileDetails(!showProfileDetails)} className="flex items-center gap-2 px-4 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all bg-primary text-white">{isLocked ? <Lock className="w-3.5 h-3.5" /> : (showProfileDetails ? <X className="w-3.5 h-3.5" /> : <Pencil className="w-3.5 h-3.5" />)}{isLocked ? t('tap_to_unlock') : (showProfileDetails ? t('close_edit') : t('edit_profile'))}</button></div>
               {showProfileDetails && !isLocked && (<div className="animate-slide-up space-y-4 pt-1"><div className="flex flex-col gap-3"><IOSInput label={t('child_name_label')} icon={User} value={editingProfile.name} onChange={(e: any) => setEditingProfile({...editingProfile, name: e.target.value})} /><button onClick={handleSaveProfile} disabled={isSavingProfile} className="w-full py-4.5 bg-primary text-white font-black rounded-3xl flex items-center justify-center gap-4 uppercase tracking-[0.25em]">{t('save_changes')}</button><button onClick={() => onDeleteProfile(editingProfile.id!)} className="w-full py-3.5 bg-rose-50 dark:bg-rose-900/10 text-rose-500 font-black rounded-3xl uppercase tracking-[0.2em]">{t('delete_profile')}</button></div></div>)}
            </div>
          </section>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 px-1">
             <button onClick={() => setView('MEMORIES')} className="bg-white dark:bg-slate-800 p-5 rounded-[32px] border border-slate-100 dark:border-slate-700 shadow-sm text-left flex flex-col justify-between h-32 group transition-all"><div className="w-9 h-9 rounded-2xl bg-rose-50 dark:bg-rose-900/20 flex items-center justify-center text-rose-500 group-hover:scale-110">{isLocked ? <Lock className="w-4.5 h-4.5" /> : <ImageIcon className="w-4.5 h-4.5" />}</div><div><h3 className="font-black text-slate-800 dark:text-white text-base mb-1">{memories.length}</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('memories')}</p></div></button>
             <button onClick={() => setView('GROWTH')} className="bg-white dark:bg-slate-800 p-5 rounded-[32px] border border-slate-100 dark:border-slate-700 shadow-sm text-left flex flex-col justify-between h-32 group transition-all"><div className="w-9 h-9 rounded-2xl bg-teal-50 dark:bg-teal-900/20 flex items-center justify-center text-teal-500 group-hover:scale-110">{isLocked ? <Lock className="w-4.5 h-4.5" /> : <Activity className="w-4.5 h-4.5" />}</div><div><h3 className="font-black text-slate-800 dark:text-white text-base mb-1">{growthData.length}</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Growth</p></div></button>
             <button onClick={() => setView('STORIES')} className="bg-white dark:bg-slate-800 p-5 rounded-[32px] border border-slate-100 dark:border-slate-700 shadow-sm text-left flex flex-col justify-between h-32 group transition-all"><div className="w-9 h-9 rounded-2xl bg-violet-50 dark:bg-violet-900/20 flex items-center justify-center text-violet-500 group-hover:scale-110">{isLocked ? <Lock className="w-4.5 h-4.5" /> : <BookOpen className="w-4.5 h-4.5" />}</div><div><h3 className="font-black text-slate-800 dark:text-white text-base mb-1">{stories.length}</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Stories</p></div></button>
          </div>
        </div>
      )}
      {view === 'STORIES' && (isLocked ? <LockedScreen /> : (
        <div className="space-y-4 animate-fade-in pb-32 px-1">
          <div className="flex items-center justify-between mb-4"><h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-widest">My Ebooks</h3><BookOpen className="w-6 h-6 text-violet-500" /></div>
          {stories.map(s => (
            <div key={s.id} onClick={() => onStoryClick(s)} className="bg-white dark:bg-slate-800 p-5 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm text-left relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all">
          ...
          {stories.length === 0 && <div className="py-20 text-center opacity-30 flex flex-col items-center gap-3"><BookOpen className="w-12 h-12"/><p className="text-xs font-black uppercase">No Ebooks Found</p></div>}
        </div>
      ))}
    </div>
  );
