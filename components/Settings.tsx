...
/* Added Settings as SettingsIcon to avoid collision with Settings component name */
...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const unsyncedMemories = memories.length - syncedMemories;
    ...
    <div className="max-w-4xl mx-auto px-2">
      ...
      {view === 'MAIN' && (
        <div className="animate-fade-in space-y-5 pb-32">
          ...
          {/* Cloud Stats Row */}
          <section className="bg-white dark:bg-slate-800 rounded-3xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-between">
             <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                   ...
                   <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">{isGuestMode ? 'Saved Locally' : `${syncedMemories}/${memories.length} Synced`}</p>
                </div>
             </div>
             <div className="flex gap-1.5">
                <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-700/50 rounded-xl flex items-center gap-2">
                   <HardDrive className="w-3.5 h-3.5 text-slate-400" />
                   <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{memories.length + growthData.length} Items</span>
                </div>
             </div>
          </section>
          ...
          <section className="bg-white dark:bg-slate-800 rounded-[32px] overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 p-5">
            <div className="flex items-center justify-between mb-4 px-1">
                <div className="flex items-center gap-2.5">
                   ...
                   <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{t('about_child')}</h3>
                </div>
            ...
            {/* Profiles List - More Compact */}
            <div className="flex gap-3 overflow-x-auto pb-4 px-1 no-scrollbar border-b border-slate-50 dark:border-slate-700/50 mb-5">
                {profiles.map(p => (
                    <button key={p.id} onClick={() => onProfileChange(p.id!)} className={`flex-shrink-0 flex flex-col items-center gap-2 group transition-all duration-300 ${p.id === activeProfileId ? 'scale-105' : 'opacity-40'}`}>
            ...
            <div className="space-y-4">
               <div className="flex items-center justify-between">
                  <div className="text-left">
                     ...
                     <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">{calculateAge(currentProfile?.dob || '')}</p>
                  </div>
                  <button onClick={() => setShowProfileDetails(!showProfileDetails)} className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${showProfileDetails ? 'bg-slate-100 dark:bg-slate-700 text-slate-500' : 'bg-primary text-white shadow-lg shadow-primary/20'}`}>
                     {showProfileDetails ? <X className="w-3.5 h-3.5" /> : <Pencil className="w-3.5 h-3.5" />}
          ...
          <section className="bg-white dark:bg-slate-800 rounded-[32px] overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700 divide-y divide-slate-50 dark:divide-slate-700/50">
            ...
            <div className="p-5 flex items-center justify-between">
               ...
               <div className="flex gap-2">
                  {passcode ? (
                    <button onClick={onPasscodeChange} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-indigo-500"><Pencil className="w-4 h-4" /></button>
            ...
            <div className="p-5 flex items-center justify-between">
               ...
               <button onClick={() => setLanguage(language === 'mm' ? 'en' : 'mm')} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-[10px] font-black rounded-xl uppercase tracking-wider">{language === 'mm' ? 'English' : 'မြန်မာ'}</button>
            </div>
          </section>
              ...
              <div className="space-y-4">
                 {growthData.map(g => (
                    <div key={g.id} className="bg-white dark:bg-slate-800 p-5 rounded-3xl flex items-center justify-between border border-slate-50 dark:border-slate-700 shadow-sm">
                       ...
                       <button onClick={() => onDeleteGrowth(g.id!)} className="p-3.5 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-2xl transition-all"><Trash2 className="w-6 h-6" /></button>
                    </div>
                 ))}
              </div>
           </div>
      ...
      {view === 'REMINDERS' && (
         <div className="space-y-8 animate-fade-in pb-32 px-1">
            ...
            <div className="space-y-4">
               {remindersList.map(r => (
                  <div key={r.id} className="bg-white dark:bg-slate-800 p-6 rounded-3xl flex items-center justify-between border border-slate-50 dark:border-slate-700 shadow-sm">
                     <div className="text-left">
